...
* **Schema migrations are append-only.** `SCHEMA_VERSION` (in `js/core.js`) bumps when the data shape changes; add a new step to the migrations table, never edit an existing step. The current version is 7.
